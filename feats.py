...
def batch_feat(cur_batch, model):
    ...
    image_batch = torch.FloatTensor(image_batch).cpu()
    ...
    for i, (path, idx) in enumerate(input_paths):
        ...
        if len(cur_batch) == args.batch_size:
            ...
            for j in range(feats.shape[0]):
                np.savez(feats_npz_path + str(ix) + '.npz', x=feats[j].reshape(100, 2048).transpose(1,0))
                ix += 1
...
if __name__ == '__main__':
    train_images_path = 'static/assets/images/'
    #val_images_path = './raws/images/val/'
    #test_images_path = './raws/images/test/'
    train_feats_npz_path = 'static/assets/npz/'
...
#4/1AY0e-g5dNAaFe9xZUrDNAIXirUnk2WgyMoMJhyFW3blhxNYRsww5BlfBlnE